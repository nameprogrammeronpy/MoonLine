// Mento - Mental Health App JavaScript
...
// Particle Effects - Calming floating particles
...
// Fluid Canvas - работает везде, даже поверх текста
function initFluidCanvas() {
    ...
    // Курсор работает везде - canvas ловит все события мыши
    ...
    function pointerPrototype() {
        ...
        this.down = false;
        this.moved = false;
        // Calming colors - soft purples and blues for mental health theme
        this.color = [0.6, 0.4, 0.9];
    }
    ...
    // Initial calm splats with calming colors
    multipleSplats(5);
    update();
    ...
    function multipleSplats(amount) {
        // Calming color palette for mental health theme
        const calmColors = [
            [0.48, 0.41, 0.93],  // Purple
            [0.42, 0.56, 0.72],  // Calm Blue
            [0.79, 0.66, 0.72],  // Soft Pink
            [0.61, 0.54, 0.70],  // Lavender
            [0.55, 0.76, 0.79],  // Teal
            [0.86, 0.79, 1.0],   // Light Purple
        ];

        for (let i = 0; i < amount; i++) {
            const color = calmColors[Math.floor(Math.random() * calmColors.length)];
            const x = canvas.width * Math.random();
            const y = canvas.height * Math.random();
            const dx = 300 * (Math.random() - 0.5); // Slower movement
    ...
    // Calming color generation
    function getCalmColor() {
        const calmColors = [
            [0.48, 0.41, 0.93],
            [0.42, 0.56, 0.72],
            [0.79, 0.66, 0.72],
...
// Курсор работает везде - отслеживаем движение на всём window
